...
export default function FilterPanel() {
  ...
  return (
    <>
      ...
      <div
        className={`drawer-backdrop ${isFilterOpen ? 'open' : ''}`}
        onClick={() => closeFilter()}
        style={{
      ...
      <div
        className={isMobile ? `filter-drawer ${isFilterOpen ? 'open' : ''}` : 'filter-panel'}
        style={{
          width: '100%',
          maxWidth: '360px',
        ...
        {isMobile && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <button onClick={() => closeFilter()} style={{ background: 'none', border: 'none', fontSize: '1.25rem', cursor: 'pointer' }} aria-label="Close filters">
              <i className="fas fa-times"></i>
            </button>
          </div>
        )}

        {/* Lock body scroll & handle ESC when drawer open on mobile */}
...
// Small utility component to lock scroll and handle ESC key
function ScrollLock() {
  useEffect(() => {
